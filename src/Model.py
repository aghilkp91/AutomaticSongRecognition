...
import os, threading, gc, multiprocessing
...
class Audio(object):
    ...
    # multi-threads
    def startInsertFingerprints(self, thread_num=ASRConfig.mysql_max_connection,
                                insert_num=ASRConfig.mysql_insert_number):
        dbs = self.db()
        ...
        while True:
            if (st + 1) * insert_num > len(fingerprints):
                ...
                total_threads.append(
                    _insertFingerprints(st, self.db, self.id, fingerprints[st * insert_num:(st + 1) * insert_num:]))
            st += 1
        st = 0
...
class _insertFingerprints(threading.Thread):
    ...
    def run(self):
        ...
        dbs = ss()
        dbs.execute(Database.Fingerprints.__table__.insert(),
                    [{"song_id": self.song_id, "fingerprint": fingerprint, "offset": int(offset)} for
                     fingerprint, offset in self.data])
        dbs.commit()
        dbs.close()
        ss.remove()
